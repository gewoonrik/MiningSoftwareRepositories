...
object App {
  ...
  def main(args : Array[String]) {
    ...
    try {
      ...
      // Execute Query
      val rs = statement.executeQuery("SELECT COUNT(*) as count, CAST(posts.CreationDate AS DATE) as dag FROM posts INNER JOIN posttags ON posts.id = posttags.PostId WHERE posttags.TagId = 72270 GROUP BY dag")

      // Iterate Over ResultSet
      println("starting writing")
      val writer = new PrintWriter(new File("..\\results\\posts_per_day_total.txt" ))
      while (rs.next) {
